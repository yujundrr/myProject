...
*	修改记录 :
*		版本号  日期        作者    说明
*       v1.0    2012-07-06 yujun  ST固件库V3.5.0版本。
*		v1.1    2012-10-22 yujun  增加4点校准
*		v1.2    2012-11-07 yujun  解决4点校准的XY交换分支的bug
*		v1.3    2012-12-17 yujun  触摸校准函数增加入口参数:等待时间
*		V1.4    2013-07-26 yujun  更改 TOUCH_DataFilter() 滤波算法
*		V1.5    2013-07-32 yujun  修改TOUCH_WaitRelease(),计数器需要清零
*		V1.6    2014-10-20 yujun
...
*		V1.7    2015-01-02 yujun  计划将触摸扫描由1ms一次修改为10ms一次。未定。
*				2015-04-21 yujun 修改 TOUCH_InitHard() 函数。GT811_InitHard() 执行后直接return
*		V1.8	2015-10-30 yujun 增加 4.3寸电容触摸 FT5x06
*					(1) 添加 void TOUCH_CapScan(void) 函数
*		V2.0	2018-01-6 TOUCH_PutKey() 增加GT911，GT811，FT5X06支持
*		V2.1	2018-09-08 yujun 移植到STM32H7平台。
*					- 电阻触摸参数单独存放到eeprom固定地址， 16K字节最后256字节空间. 和APP的param.c独立
*					- 同时支持2点校准和4点校准，不采用条件编译了。
*					- void TOUCH_Calibration(uint8_t _PointCount); 增加形参
*
*	Copyright (C), 2015-2020, ************yujun*************
*
*********************************************************************************************************
*/
...
//#define TP_PARAM_FLASH_ADDR ADDR_FLASH_SECTOR_10 
