...
*	修改记录 :
*		版本号  日期       作者    说明
*		V1.0    2013-02-01 yujun  正式发布
*		V1.1    2013-06-09 yujun  FiFo结构增加TxCount成员变量，方便判断缓冲区满; 增加 清FiFo的函数
*		V1.2	2014-09-29 yujun  增加RS485 MODBUS接口。接收到新字节后，直接执行回调函数。
*		V1.3	2015-07-23 yujun  增加 UART_T 结构的读写指针几个成员变量必须增加 __IO 修饰,否则优化后
*					会导致串口发送函数死机。
*		V1.4	2015-08-04 yujun  解决UART4配置bug  GPIO_PinAFConfig(GPIOC, GPIO_PinSource11, GPIO_AF_USART1);
*		V1.5	2015-10-08 yujun  增加修改波特率的接口函数
*		V1.6	2018-09-07 yujun  移植到STM32H7平台
*		V1.7	2018-10-01 yujun  增加 Sending 标志，表示正在发送中
*		V1.8	2018-11-26 yujun  增加UART8，第8个串口
*
*	Copyright (C), 2015-2030, ************yujun*************
*
*********************************************************************************************************
*/
...
static void UartVarInit(void)
{
	...
	g_tUart3.SendBefor = 0;		/* 发送数据前的回调函数RS485_SendBefor; */
	g_tUart3.SendOver = 0;			/* 发送完毕后的回调函数RS485_SendOver; */
	g_tUart3.ReciveNew = 0;		/* 接收到新数据后的回调函数RS485_ReciveNew; */
