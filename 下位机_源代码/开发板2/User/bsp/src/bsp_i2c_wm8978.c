...
*	文件名称 : bsp_wm8978.h
*	版    本 : V1.5
*	说    明 : WM8978音频芯片和STM32 I2S底层驱动。在   STM32-V5开发板上调试通过。
*
*	修改记录 :
*		版本号  日期        作者     说明
*		V1.0    2013-02-01 yujun  正式发布
*		V1.1    2013-06-12 yujun  解决单独Line 输入不能放音的问题。修改 wm8978_CfgAudioPath() 函数
*		V1.2    2013-07-14 yujun  增加设置Line输入接口增益的函数： wm8978_SetLineGain()
*		V1.3    2015-10-18 yujun  移植到STM32F429，改动很大。I2S接口修改为SAI音频接口。
*							-  wm8978_CfgAudioIF() 函数的字长形参，增加20bit
*		V1.4	2016-01-24 yujun   更正AUDIO_Play 函数内bug，变量未初始化就使用了。
*		V1.5	2016-03-25 yujun   更正AUDIO_Record函数内bug，变量未初始化就使用了。解决录音需要2次的问题
*
...
/* 
	   STM32-V7开发板---  SAI接口 I2S总线传输音频数据口线
...
*						   开发板硬件不支持 I2S_Mode_SlaveTx、I2S_Mode_SlaveRx 模式，这需要WM8978连接
*						外部振荡器
*	返 回 值: 无
*********************************************************************************************************
*/
