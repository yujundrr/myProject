...
*	修改记录 :
*		版本号   日期         作者        说明
*		V1.0    2021-01-30      yujun    正式发布
*
*	Copyright (C), 2021-2030, ************yujun*************
*
*********************************************************************************************************
*/
#include "includes.h"
		...
		if (comGetChar(COM1, &cmd))	/* 从串口读入一个字符(非阻塞方式) */
		{
			...
			switch (cmd)
			{
				...
				case '3':
					...
					ReadFileData();		/* 读取根目录下yujun.txt的内容 */
					break;
...
static void DispMenu(void)
{
	...
	printf("2 - 创建一个新文件yujun.txt\r\n");
	printf("3 - 读yujun.txt文件的内容\r\n");
	printf("4 - 创建目录\r\n");
	printf("5 - 删除文件和目录\r\n");
	printf("6 - 读写文件速度测试\r\n");
	printf("a - 打开SD卡模拟U盘\r\n");
...
*********************************************************************************************************
*	函 数 名: CreateNewFile
*	功能说明: 在SD卡创建一个新文件，文件内容填写“www.yujun.com”
*	形    参：无
*	返 回 值: 无
*********************************************************************************************************
*/
static void CreateNewFile(void)
{
    ...
    /* 创建文件 */
    status =  fx_file_create(&sdio_disk, "yujun.txt");
    ...
    /* 打开文件  */
    status =  fx_file_open(&sdio_disk, &fx_file, "yujun.txt", FX_OPEN_FOR_WRITE);
    ...
    if (status == FX_SUCCESS)
    {
        printf("yujun.txt 文件写入成功\r\n");        
    }
    else
    {
        printf("yujun.txt 文件写入失败 %d\r\n", status); 
    }
...
*********************************************************************************************************
*	函 数 名: ReadFileData
*	功能说明: 读取文件yujun.txt前128个字符，并打印到串口
*	形    参：无
*	返 回 值: 无
*********************************************************************************************************
*/
static void ReadFileData(void)
{
    ...
    /* 打开文件  */
    status =  fx_file_open(&sdio_disk, &fx_file, "yujun.txt", FX_OPEN_FOR_READ);
...
*********************************************************************************************************
*	函 数 名: DeleteDirFile
*	功能说明: 删除SD卡根目录下的 yujun.txt 文件和 Dir1，Dir2 目录
*	形    参：无
*	返 回 值: 无
*********************************************************************************************************
*/
static void DeleteDirFile(void)
{
	...
	/* 删除文件 yujun.txt */
    status = fx_file_delete(&sdio_disk, "yujun.txt");
    
    if (status == FX_SUCCESS)
	{
		printf("删除目录yujun.txt成功\r\n");
	}
	else if (status == FX_NOT_FOUND)
	{
		printf("没有发现文件或目录 :%s\r\n", "yujun.txt");
	...
	{
		printf("删除yujun.txt失败(错误代码 = %d) 文件只读或目录非空\r\n", status);
	}   
